...
class MyPetsSection extends Component<{}, IState> {
  ...
  _onHandleInputChange = (field: string, value: any) => {
    ...
    const pets_owned_copy: Array<Pet> = _.cloneDeep(this.state.pets_owned);
  ...
  componentDidUpdate = () => {
    if (
      this.context.currentUser.pets_owned.length !==
      this.state.pets_owned.length
    ) {
  ...
  _renderPetProfile(index: number, pet: Pet, totalPets: number) {
      ...
      <div key={index}>
        <Form>
          <Form.Group>
            ...
            <Form.Control
              ...
              onChange={(e) =>
                this._onHandleInputChange(`[${index}].name`, e.target.value)
              }
            />
          </Form.Group>
          <Form.Group>
          ...
          <Form.Group>
            ...
            <Form.Control
              type="text"
              value={
                "special_requirements" in pet ? pet.special_requirements : ""
              }
              onChange={(e) =>
                this._onHandleInputChange(
                  `[${index}].special_requirements`,
                  e.target.value
                )
              }
            />
          ...
          <Form.Group>
            ...
            <DatePicker
              value={
                "date_of_birth" in pet
                  ? new Date(pet.date_of_birth)
                  : new Date(Date.now())
              }
              format="y-MM-dd"
              onChange={(date) =>
                this._onHandleInputChange(`[${index}].date_of_birth`, date)
              }
            />
          </Form.Group>
          <Form.Group>
            ...
            <Form.Check
              ...
              onChange={(e) =>
                this._onHandleInputChange(`[${index}].gender`, MALE)
              }
            />
            <Form.Check
              type="radio"
              ...
              onChange={(e) =>
                this._onHandleInputChange(`[${index}].gender`, FEMALE)
              }
            />
          </Form.Group>
        </Form>
        {index !== totalPets - 1 ? (
          <div>
            <Button
              variant="primary"
              onClick={() => this._updatePet(pet)}
              style={{ marginRight: "20px" }}
            >
              Update Pet
            </Button>
            <Button variant="danger" onClick={() => this._deletePet(pet.name)}>
              Delete Pet
            </Button>
            <hr style={{ margin: "50px 0", border: "1px solid black" }} />
          </div>
        ) : (
          <Button variant="success" onClick={() => this._addNewPet(index)}>
            Add New Pet
          </Button>
        )}
      </div>
