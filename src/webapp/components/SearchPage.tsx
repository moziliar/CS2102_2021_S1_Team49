...
class SearchPage extends Component<{}, IState> {
                    ...
                    <Row>
                        <Col xs={ 7 } className="section">
                            ...
                            { careTakers?.length === 0 || careTakers === null    
                                ? <h3 style={{ 'position': 'absolute', 'top': '50%', 'left': '50%', 'transform': 'translate(-50%, -50%)' }}>There is 0 result found.</h3>
                                : <div>
                                        { careTakers.map((careTaker, index) => this._renderCareTakerCard(index, careTaker)) }
                                  </div>
                            }
                        </Col>
        ...
        return (
            <Modal
                ...
                <Modal.Body>
                    <Form>
                        <Form.Group>
                            <Form.Label><strong>Choose your pet category:</strong></Form.Label><br />
                            { categories?.map(c => {
                                return (
                                    <Form.Check 
                                        key={ c.name }
                                        ...
                                        onChange={ e => this._onHandleInputChange(CATEGORY, c.name) }/>
                                );
                            }) }
                        </Form.Group>
                        <Form.Group>
                            <Form.Label><strong>Minimum rating for Care Taker:</strong></Form.Label><br />
                            <Form.Control 
                                type="number"
                                min={ 0 }
                                max={ 5 }
                                value={ formData[RATING] }
                                onChange={ (e) => this._onHandleInputChange(RATING, e.target.value) } />
                        </Form.Group>
                        <Form.Group>
                            <Form.Label><strong>What is your maximum budget(per night)</strong></Form.Label><br />
                            <Form.Control 
                                type="number"
                        ...
                        <Button 
                            ...
                            disabled={ !formData[CATEGORY] || !formData[START_DATE] || !formData[END_DATE] || formData[PRICE] === "" || !formData[RATING] === "" }>
                                Search Now
                        </Button>
                    </Form>
                </Modal.Body>
    ...
    _renderCareTakerCard = (index: number, careTaker: CareTaker) => {
        return (
            <div key={ careTaker.name } className="caretaker-card" onClick={ () => this._selectCareTaker(index) }>
                <Row style={{ 'margin': '20px' }}>
                    <Col xs={ 3 } className="detail">
                        <img src={ careTaker.pic_url } style={{ 'height': '100px', 'width': '100px', 'marginRight': '10px' }}/>
                    </Col>
                    <Col xs={ 9 } className="detail">
                        <h5>{ index + 1 }. { careTaker.name } ({ careTaker.rating } / 5.0)</h5>
                ...
                <div>
                    ...
                    { reviews 
                        ? reviews.map((review, index) => {
                            return (
                                <div key={ index }>
                                    <p>{ index + 1 }. { review.owner_name } - { review.review } ({ review.rating }/5.0)</p>
                                </div>                            
                            );
                          })
                        : null
